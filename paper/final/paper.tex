...
robustness checks, on frozen checkpoints from $0.5$B to $70.6$B, scoring by
direct next-token likelihood over verified single-token candidates so that no
free text is parsed.
...
strict, too: on the Qwen checkpoints every arm whose record never displays the
query's predicate returns $d' = 0$, so high accuracy with a full certificate
describes a faithful transcriber rather than a reasoner.
...
on $189$, $186$ and $192$ of $192$ items, and the strongest abstains on $0$ of
$192$ broken certificates.
Mechanism claims about solver assistance do not transfer between checkpoints.
The failure to check the solver does.
...
decides. We apply it to fourteen models, thirteen of which the analysis interprets
\citep{team2024qwen,yang2025qwen3,grattafiori2024llama,team2025gemma,microsoft2025phi,team2024olmo,allal2025smollm}.
...
$38{,}424$ responses it returns a different answer almost every time it
