...
Supporting material for the results in the body. \Cref{tab:panels} inventories the sealed panels; \cref{tab:edges} gives the four edges of the $2\times2$ with their adjusted $p$ values; \cref{tab:detection} the abstention deltas of \cref{sec:detection}. The four figures plot results whose values are already tabulated above: the ten arms (\cref{fig:arms}), the validity share across checkpoints (\cref{fig:replication}), the three-class response distribution (\cref{fig:detection}), the order profile (\cref{fig:order}) and the substrate ladder (\cref{fig:size}).
