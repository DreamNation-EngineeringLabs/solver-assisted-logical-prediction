...
robustness checks: $34{,}584$ scored responses, re-derivable from
released receipts.
...
attributes. Cells give correct items of 192 for Experiment~1. The dashed
...
candidates so that no free text is parsed, with two robustness checks
...
What is new is the instrument: an answer-evidence decomposition that separates
solver-supplied answers from solver-supplied state, a surface-matched invalidity
control certified per item, and a forward-closure certifier for underdetermined
items. We apply it to twelve models
...
program a solver executes \citep{lyu2023faithful}. Neurosymbolic work puts the formal step
outside the model altogether: translating into a symbolic language and repairing from solver errors \citep{pan2023logiclm}, parsing into
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.92\textwidth]{figures/validity_share_replication.png}
...
and all $34{,}584$ verified with zero failures. The count is not typed into the manuscript: it is
...
Supporting material for the results in the body. \Cref{tab:panels} inventories the sealed panels and \cref{tab:detection} gives the abstention deltas of the three-candidate rescoring. The four figures plot results whose values are already tabulated above: the ten arms (\cref{fig:arms}), the three-class response distribution (\cref{fig:detection}), the gap-versus-direction profile (\cref{fig:order}) and the substrate ladder (\cref{fig:size}).
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.92\textwidth]{figures/detection_response_distribution.png}
