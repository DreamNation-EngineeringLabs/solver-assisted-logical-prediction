...
interpretable: one arm scored $50.0\%$ on a balanced panel at $d' = 0.00$, so
...
\citep{tafjord2020proofwriter} and nine adversarial classes; (iv) evidence that
apparent use of proof state can be surface exploitation, that the behaviour is
completion rather than detection, and that no checkpoint resists a corrupted
certificate; and (v) a sweep over eleven models
...
an attribution question needs. We generate items after model release with
per-item nonce vocabularies and construct the matched invalid, misleading,
order-scrambled and depth-truncated certificates the public sets do not provide.
A public open-world corpus is used only to check the forward-closure certifier;
no item from it is scored by any model here.
