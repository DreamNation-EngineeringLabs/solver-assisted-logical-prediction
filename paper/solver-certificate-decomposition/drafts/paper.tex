...
Both lines leave our question open. The first intervenes on \emph{model-generated}
traces, asking whether the model's own account is honest; ours is
\emph{externally supplied, formally verified} state, where the question inverts to
whether the model tracks a derivation it did not produce, and where a null means
something different --- ignoring a mistake in one's own trace is unfaithful,
whereas following a corrupted external certificate is unsafe. The second reports
end-to-end accuracy, which cannot separate the solver's \emph{answer} from its
\emph{state}, and assumes the solver correct, so behaviour under
wrong-but-well-formed output is unmeasured --- though the model side is
demonstrably not neutral to context, with irrelevant material degrading accuracy
\citep{shi2023large} and coherent evidence overriding parametric knowledge
\citep{xie2023adaptive}. We decompose the two channels, and add both a
surface-matched invalid certificate and one that is valid-looking but points the
wrong way.
...
RuleTaker \citep{clark2020transformers}, ProofWriter, which added generated proofs
and open-world semantics with an explicit unknown class
\citep{tafjord2020proofwriter}, PrOntoQA, whose individual steps are usually
correct while proof planning is not \citep{saparov2022language}, and FOLIO's
human-written premises \citep{han2022folio}. Order effects are large in deductive
prompting \citep{chen2024premise}, and these sets are public and predate the
models evaluated on them, so contamination cannot be excluded
\citep{deng2024investigating,golchin2023time}; counterfactual variants of familiar
tasks show how much apparent competence rests on familiarity
\citep{wu2023reasoning}. They supply items and proofs but not the matched
adversarial certificate variants an attribution question needs, so we generate
items after model release with per-item nonce vocabularies and build the matched
invalid, misleading, order-scrambled and depth-truncated certificates ourselves.
...
\citep{kadavath2022language,yin2023large}. Evaluation in solver-augmented
reasoning has not absorbed this, and bare accuracy remains the reported quantity:
on a balanced panel a degenerate single-label responder is indistinguishable from
a chance-level competent one, and both from a partial discriminator with a
shifted criterion. Signal detection theory separates exactly these
\citep{macmillan1991detection,stanislaw1999calculation}, so we report $d'$ and
criterion $c$ alongside accuracy for every arm, with paired significance from the
exact McNemar test \citep{mcnemar1947note} and interval estimates from the
bootstrap \citep{efron1987better}. Two concrete cases here motivate the practice
