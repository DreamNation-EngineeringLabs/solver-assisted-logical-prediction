...
(\cref{fig:factorial}), a no-material baseline outside the square:
...
To isolate validity we cut one intermediate link, so the displayed lines no
longer reach the query, while holding the surface fixed against the matched
\textsc{truncate\_1} arm: the same query-entity occurrence count (audited per
item), the same query predicate in the final rule, the same line count and the
same fact/rule/literal sequence. Every rule displayed is a real rule of the
...
\end{align}
Four companion arms separate the rest, and \cref{tab:arms} lists them:
...
\textsc{shuffled} reorders \textsc{truncate\_1}'s lines;
\textsc{truncate\_2} and \textsc{truncate\_3} withhold more steps; and
\textsc{misleading} supplies a valid-looking chain whose single fabricated final
rule establishes the query's negation, leaving the true answer unchanged so that
following the supplied state means answering incorrectly.
...
Four experiments and two robustness checks, all on rule-chaining queries over
small theories of facts and universally quantified rules. \Cref{tab:panels}
names them and lists their panels. The sealed
panels are generated after model release with per-item invented vocabularies,
are mutually disjoint, and fix derivation depth at four. The public corpus
validates the certifier only: no item from it is scored by any model here.

\begin{table}[t]
\centering
\caption{Datasets. The sealed panels are nonce-generated and disjoint; the
public corpus validates the certifier only. The abstention rescoring reuses the validity control's
...
\midrule
\texttt{b14} & entailed / contradicted & 192 & the reanalysis \\
\texttt{b15} & entailed / contradicted & 192 & the validity control \\
\texttt{b16} & entailed / contradicted / undetermined & 192 & the model sweep \\
\texttt{b17}, \texttt{b17b} & \texttt{b15} items, three candidates & 192 & the abstention rescoring \\
...
weights. The sweep scores ten instruction-tuned models from $0.5$B to $7.6$B
...
The reanalysis inherits a null on its prespecified primary contrast,
$\textsc{proof\_prefix} - \textsc{conclusion\_only} = -4.7$pp (exact McNemar
$p = 0.078$). Read through the square, that contrast is the diagonal: it moves
both factors at once, so its null attributes to neither.
