#!/usr/bin/env python3
"""Addendum in response to peer review: an interval on the validity share.
...
def main() -> None:
    ...
    out = {
        "version": "b15-share-interval-v2",
        "n_entailed": n,
        "point": {"total": point_total, "validity": point_valid,
                  "surface": (sum(bc) - sum(ir)) / n, "share": point_share},
        "validity_share_percentile95": [lo, hi],
        "surface_component_percentile95": [slo, shi],
        "bootstrap": {"seed": SEED, "resamples": B, "method": "paired item bootstrap of the ratio, percentile interval"},
        "note": ("v1 bootstrapped the numerator only. Resampling items and recomputing the "
                 "whole ratio is the correct interval for a share. The interval is a "
                 "percentile interval, not BCa: the statistic is a ratio of paired "
                 "differences, for which the acceleration term is not defined here."),
        "holm": ("NOT computed here. Holm adjustment for the contrasts the paper prints is "
                 "in analyse_cognitive_binary_certificate_factorial_b15.py, over the "
                 "declared secondary family."),
    }
    p = root / "results/b15_share_interval_v2.json"
    p.write_text(json.dumps(out, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"  validity share  {point_share*100:.1f}%   95% percentile [{lo*100:.1f}, {hi*100:.1f}]")
